...
func testAccCheckIBMSchematicsJobDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if err == nil {
			return fmt.Errorf("Complete Job with user inputs and system generated data. still exists: %s", rs.Primary.ID)
		} else if response.StatusCode != 404 {
			return fmt.Errorf("Error checking for Complete Job with user inputs and system generated data. (%s) has been destroyed: %s", rs.Primary.ID, err)
		}
	}
