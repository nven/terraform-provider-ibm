...
func resourceIBMSchematicsActionCreate(d *schema.ResourceData, meta interface{}) error {
	...
	action, response, err := schematicsClient.CreateAction(createActionOptions)
	if err != nil {
		log.Printf("[DEBUG] CreateAction failed %s\n%s", err, response)
		return err
	}
...
func resourceIBMSchematicsActionRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		log.Printf("[DEBUG] GetAction failed %s\n%s", err, response)
		return err
	}
	...
	if action.UserState != nil {
		...
		if err = d.Set("user_state", []map[string]interface{}{userStateMap}); err != nil {
			return fmt.Errorf("Error reading user_state: %s", err)
		}
	}
	...
	if err = d.Set("source_type", action.SourceType); err != nil {
		return fmt.Errorf("Error reading source_type: %s", err)
	}
	...
	if err = d.Set("trigger_record_id", action.TriggerRecordID); err != nil {
		return fmt.Errorf("Error reading trigger_record_id: %s", err)
	}
	if action.State != nil {
...
func resourceIBMSchematicsActionUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if hasChange {
		_, response, err := schematicsClient.UpdateAction(updateActionOptions)
		if err != nil {
			log.Printf("[DEBUG] UpdateAction failed %s\n%s", err, response)
			return err
		}
	}
...
func resourceIBMSchematicsActionDelete(d *schema.ResourceData, meta interface{}) error {
	...
	response, err := schematicsClient.DeleteAction(deleteActionOptions)
	if err != nil {
		log.Printf("[DEBUG] DeleteAction failed %s\n%s", err, response)
		return err
	}
