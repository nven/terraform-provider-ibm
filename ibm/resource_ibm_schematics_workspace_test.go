...
func testAccCheckIBMSchematicsWorkspaceDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if err == nil {
			return fmt.Errorf("WorkspaceResponse - request returned by create. still exists: %s", rs.Primary.ID)
		} else if response.StatusCode != 404 {
			return fmt.Errorf("Error checking for WorkspaceResponse - request returned by create. (%s) has been destroyed: %s", rs.Primary.ID, err)
		}
	}
