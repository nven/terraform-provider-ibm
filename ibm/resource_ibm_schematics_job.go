...
func resourceIBMSchematicsJob() *schema.Resource {
	return &schema.Resource{
		...
		Exists:   resourceIBMSchematicsJobExists,
...
func resourceIBMSchematicsJobCreate(d *schema.ResourceData, meta interface{}) error {
	...
	job, response, err := schematicsClient.CreateJob(createJobOptions)
	if err != nil {
		log.Printf("[DEBUG] CreateJob failed %s\n%s", err, response)
		return err
	}
...
func resourceIBMSchematicsJobRead(d *schema.ResourceData, meta interface{}) error {
	...
	job, response, err := schematicsClient.GetJob(getJobOptions)
	if err != nil {
	...
	if err = d.Set("command_object_id", job.CommandObjectID); err != nil {
		return fmt.Errorf("Error reading command_object_id: %s", err)
	}
	...
	if job.CommandOptions != nil {
		if err = d.Set("command_options", job.CommandOptions); err != nil {
			return fmt.Errorf("Error reading command_options: %s", err)
		}
	}
	if job.Inputs != nil {
		...
		if err = d.Set("inputs", inputs); err != nil {
			return fmt.Errorf("Error reading inputs: %s", err)
		}
	}
	if job.Settings != nil {
	...
	if job.LogSummary != nil {
		...
		if err = d.Set("log_summary", []map[string]interface{}{logSummaryMap}); err != nil {
			return fmt.Errorf("Error reading log_summary: %s", err)
		}
	}
	...
	if err = d.Set("duration", job.Duration); err != nil {
		return fmt.Errorf("Error reading duration: %s", err)
	}
	if err = d.Set("targets_ini", job.TargetsIni); err != nil {
		return fmt.Errorf("Error reading targets_ini: %s", err)
	}
	if err = d.Set("log_store_url", job.LogStoreURL); err != nil {
		return fmt.Errorf("Error reading log_store_url: %s", err)
	}
	if err = d.Set("state_store_url", job.StateStoreURL); err != nil {
		return fmt.Errorf("Error reading state_store_url: %s", err)
	}
...
func resourceIBMSchematicsJobUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	_, response, err := schematicsClient.ReplaceJob(replaceJobOptions)
	if err != nil {
		log.Printf("[DEBUG] ReplaceJob failed %s\n%s", err, response)
		return err
	}
...
func resourceIBMSchematicsJobDelete(d *schema.ResourceData, meta interface{}) error {
	...
	response, err := schematicsClient.DeleteJob(deleteJobOptions)
	if err != nil {
		log.Printf("[DEBUG] DeleteJob failed %s\n%s", err, response)
		return err
	}
...
func resourceIBMSchematicsJobExists(d *schema.ResourceData, meta interface{}) (bool, error) {
	schematicsClient, err := meta.(ClientSession).SchematicsV1()
	if err != nil {
		return false, err
	}
